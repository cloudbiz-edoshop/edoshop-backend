export const nextRetailerCode = `
CREATE OR REPLACE FUNCTION next_retailer_code()
RETURNS TEXT AS $$
DECLARE
    next_code TEXT;
    last_code TEXT;
    ...
    number INT;
BEGIN
    -- Get the last retailer code from the table, locking the row
    SELECT MAX(retailer_code) INTO last_code FROM retailers;
    ...
    ELSE
        ...
        IF last_pure_code !~ '^[A-Z]{1,2}[0-9]{2}$' THEN
            RAISE EXCEPTION 'Invalid retailer_code format: %', last_pure_code;
        END IF;
        ...
        IF number > 99 THEN
            ...
            ELSE
                ...
                IF second_letter > 'Z' THEN
                    ...
                    IF first_letter > 'Z' THEN
                        RAISE EXCEPTION 'Retailer code limit reached (ZZ99)';
                    END IF;
                END IF;
            END IF;
...
export default nextRetailerCode;
