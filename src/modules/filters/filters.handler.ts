...
export const list: AppRouteHandler<ListRoute> = async (c) => {
  ...
  // Use supplier service for listing Filters
  const result = await filtersService.listFilters({
    search,
    page,
    limit,
    sortBy,
  ...
  const filtersList: ListFiltersResponse = result.data;
...
export const create: AppRouteHandler<CreateRoute> = async (c) => {
  ...
  const result = await filtersService.createFilters({
    name,
    description,
    createdBy,
  });

  const response: CreateFiltersResponse = result;
...
export const getOne: AppRouteHandler<GetOneRoute> = async (c) => {
  ...
  const filters = await filtersService.getFiltersById(id);
  const typedResponse: GetFiltersResponse = filters;
...
export const patch: AppRouteHandler<PatchRoute> = async (c) => {
  ...
  // Use filters service to update the filters
  const updatedFilters = await filtersService.updateFilters(id, data);
...
export const removeSelected: AppRouteHandler<RemoveSelectedRoute> = async (
  ...
  // Use filters service to delete multiple filters
  await filtersService.deleteFilters(ids, deletedBy);
