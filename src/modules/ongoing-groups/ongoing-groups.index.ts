import { createRouter } from "@/lib/create-app";

import * as handlers from "./ongoing-groups.handler";
import * as routes from "./ongoing-groups.route";

const router = createRouter();

router.openapi(routes.list, handlers.list);
router.openapi(routes.create, handlers.create);
router.openapi(routes.productSummary, handlers.productSummary);
router.openapi(routes.activeColorGroups, handlers.activeColorGroups);
router.openapi(routes.adminGroups, handlers.adminGroups);
router.openapi(routes.adminGroupById, handlers.adminGroupById);
router.openapi(routes.getOne, handlers.getOne);
router.openapi(routes.patch, handlers.patch);
router.openapi(routes.approveGroup, handlers.approveGroup);
router.openapi(routes.rejectGroup, handlers.rejectGroup);
router.openapi(routes.remove, handlers.remove);
router.openapi(routes.adminCancel, handlers.adminCancel);
router.openapi(routes.undo, handlers.undo);
router.openapi(routes.ongoingRequestsByUser, handlers.ongoingRequestsByUser);
export default router;
